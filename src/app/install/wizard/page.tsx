  ...
  return (
    <div className="install-wizard-container">
      <style>{`
        ...
        .install-form-input-group .install-form-input {
          padding-right: 44px;
        }
        ...
        .install-btn-next:hover {
          transform: translateY(-1px);
          box-shadow: 0 6px 20px rgba(197, 168, 128, 0.45);
        }

        .install-btn-next:disabled {
        ...
        .install-log-msg.success {
          color: var(--status-success);
        }

        .install-log-msg.error {
          color: var(--status-error);
        }
        ...
        .install-run-status.running {
          background: rgba(197, 168, 128, 0.08);
          border: 1px solid rgba(197, 168, 128, 0.15);
        }

        .install-run-status.success {
          background: rgba(16, 185, 129, 0.08);
          border: 1px solid rgba(16, 185, 129, 0.2);
        }

        .install-run-status.error {
          background: rgba(239, 68, 68, 0.08);
          border: 1px solid rgba(239, 68, 68, 0.2);
        }

        .install-run-status-text {
          font-size: 0.9rem;
          font-weight: 500;
        }

        .install-run-status.running .install-run-status-text {
          color: var(--accent-gold);
        }

        .install-run-status.success .install-run-status-text {
          color: var(--status-success);
        }

        .install-run-status.error .install-run-status-text {
          color: var(--status-error);
        }
            ...
            return (
              <div key={s.num} style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <div className="install-step-item">
                  <div
                    className={`install-step-circle ${isActive ? 'active' : ''} ${isDone ? 'done' : ''}`}
                  >
                    {isDone ? <CheckCircle2 size={16} /> : <Icon size={16} />}
                  </div>
                  <span className={`install-step-label ${isActive ? 'active' : ''}`}>
                    {s.label}
                  </span>
                </div>
              </div>
        ...
        {step === 1 && (
          <div>
            ...
            <div className="install-form-group">
              <label className="install-form-label">DATABASE_URL</label>
              <input
          ...
          <div>
            ...
            <div className="install-form-group">
              ...
              <div className="install-form-input-group">
                ...
                />
                <button
                  className="install-form-toggle-btn"
            ...
            {runStatus === 'error' && (
              <div className="install-success-actions">
                <button
                  ...
                  onClick={() => { setStep(1); setRunStatus('idle'); setLogs([]); setErrorMsg(''); }}
                >
                  <ArrowLeft size={16} />
                  Voltar e Corrigir
                </button>
                <button
                  className="install-btn-next"
                  onClick={runInstall}
                >
                  Tentar Novamente
                </button>
