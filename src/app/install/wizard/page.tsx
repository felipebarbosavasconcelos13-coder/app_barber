...
import { Database, Settings, ArrowRight, ArrowLeft, Loader2, CheckCircle2, AlertCircle, Eye, EyeOff, Search, Cloud, Plus, ChevronDown } from 'lucide-react';

type Step = 1 | 2 | 3;
type RunStatus = 'idle' | 'running' | 'success' | 'error';
interface LogEntry { time: string; message: string; type: 'info' | 'success' | 'error'; }
interface Org { slug: string; name: string; }
interface Proj { ref: string; name: string; status?: string; region?: string; }
...
export default function InstallWizardPage() {
  ...
  const [orgProjects, setOrgProjects] = useState<Proj[]>([]);
  ...
  // Auto-detect Vercel project
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hostname.endsWith('.vercel.app') && !vercelProjectId) {
      setVercelProjectId(window.location.hostname.replace('.vercel.app', ''));
    }
  }, []);

  const searchVercel = async () => {
    if (!vercelToken.trim()) return;
    setLoadingVercel(true);
    try {
      const r = await fetch('/api/install/vercel/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: vercelToken.trim() }) });
      const d = await r.json();
      if (r.ok && d.projects) { setVercelProjects(d.projects); if (d.projects.length === 1) setVercelProjectId(d.projects[0].id); }
      else setStepError(d.error || 'Token Vercel invalido');
    } catch { setStepError('Erro ao buscar projetos Vercel'); } finally { setLoadingVercel(false); }
  };
  ...
  const loadProjects = async (slug: string) => {
    ...
    } catch {} finally { setLoadingOrgProjects(false); }
  };
  ...
  const handleCreateProject = async () => {
    ...
    } catch (err: any) { setStepError(err.message); setCreatingProject(false); }
  };

  const selectProject = (ref: string) => {
    setSelectedProjectRef(ref);
    setSelectedSupabaseUrl(`https://${ref}.supabase.co`);
  };

  const validateStep1 = () => {
    if (!supabaseToken.trim()) { setStepError('Token Supabase obrigatorio.'); return false; }
  ...
  return (
    <div className="install-wizard-container">
      <style>{`
        ...
        .install-step-label { font-size: 0.73rem; color: var(--text-muted); display: none; }
        .install-step-label.active { color: var(--accent-gold); }
        .install-step-connector { width: 28px; height: 2px; background: var(--border-color); margin: 0 4px; transition: background 0.3s; }
        ...
        .install-form-input-group .install-form-input { padding-right: 42px; }
        ...
        .install-tag.warn { background: rgba(245,158,11,0.1); color: var(--status-warning); }
        ...
        .install-project-card:hover { border-color: var(--accent-gold); }
        .install-project-card.selected { border-color: var(--accent-gold); background: rgba(197,168,128,0.08); }
        .install-project-card-name { font-weight: 600; font-size: 0.9rem; color: #fff; }
        .install-project-card-detail { font-size: 0.75rem; color: var(--text-muted); }
        .install-create-box { padding: 16px; background: rgba(0,0,0,0.2); border: 1px dashed var(--border-color); border-radius: 10px; margin-top: 12px; }
        .install-create-box h4 { font-size: 0.9rem; color: #fff; margin-bottom: 12px; }
        ...
        .install-btn-create:hover { background: rgba(16,185,129,0.25); }
        .install-btn-create:disabled { opacity: 0.5; cursor: not-allowed; }
        ...
        .install-log-entry { display: flex; gap: 10px; padding: 4px 0; font-size: 0.8rem; line-height: 1.5; }
        .install-log-time { flex-shrink: 0; color: var(--text-muted); font-family: monospace; font-size: 0.75rem; }
      ...
      <div className="install-wizard-card">
        ...
        {step === 1 && (
          <div>
            ...
            <p className="install-step-desc">Cole seus tokens para configurar tudo automaticamente.</p>
            ...
            <div className="install-form-group">
              ...
              <div className="install-btn-row">
                <input className="install-form-input" type="password" value={vercelToken} onChange={e => { setVercelToken(e.target.value); setVercelProjects([]); }} placeholder="Full Account token" style={{ flex: 1 }} />
                <button className="install-btn-search" onClick={searchVercel} disabled={!vercelToken.trim() || loadingVercel}>{loadingVercel ? <Loader2 size={15} className="install-spinner" /> : <Search size={15} />}</button>
              </div>
              {vercelProjects.length > 0 && <span className="install-tag ok">{vercelProjects.length} projeto(s)</span>}
              <div className="install-form-hint"><a href="https://vercel.com/account/tokens" target="_blank" rel="noopener" style={{ color: 'var(--accent-gold)' }}>vercel.com/account/tokens</a> — Full Account (opcional).</div>
            </div>
            ...
            <div className="install-form-group">
              ...
              <div className="install-btn-row">
                <input className="install-form-input" type="password" value={supabaseToken} onChange={e => { setSupabaseToken(e.target.value); setOrgs([]); }} placeholder="Personal Access Token (PAT)" style={{ flex: 1 }} />
                <button className="install-btn-search" onClick={searchSupabase} disabled={!supabaseToken.trim() || loadingSupabase}>{loadingSupabase ? <Loader2 size={15} className="install-spinner" /> : <Search size={15} />}</button>
              </div>
              {orgs.length > 0 && <span className="install-tag ok">{orgs.length} org(s)</span>}
              <div className="install-form-hint"><a href="https://supabase.com/dashboard/account/tokens" target="_blank" rel="noopener" style={{ color: 'var(--accent-gold)' }}>supabase.com/dashboard/account/tokens</a> — PAT.</div>
            </div>
        ...
        {/* PASSO 2: Setup Supabase + Vercel + Admin */}
        {step === 2 && (
          <div>
            <h2 className="install-step-title">Configurar Projeto</h2>
            <p className="install-step-desc">Selecione ou crie um projeto Supabase e configure o restante.</p>

            {/* Org selector */}
            {orgs.length > 1 && (
              <div className="install-form-group">
                <label className="install-form-label">Organizacao Supabase</label>
            ...
            {selectedOrg && (
              <>
                <div className="install-form-group">
                  ...
                  {loadingOrgProjects ? <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)' }}><Loader2 size={20} className="install-spinner" /></div> :
                    orgProjects.length === 0 ? <div style={{ padding: '14px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>Nenhum projeto nesta organizacao.</div> :
                ...
                <div className="install-create-box">
                  ...
                  <div className="install-form-group">
                    <input className="install-form-input" type="text" value={newProjectName} onChange={e => setNewProjectName(e.target.value)} placeholder="Nome do projeto" />
                  </div>
                  <div className="install-form-group">
                    <input className="install-form-input" type="password" value={newProjectDbPass} onChange={e => setNewProjectDbPass(e.target.value)} placeholder="Senha do banco (min 12 caracteres)" />
                  </div>
                  <div className="install-form-group">
                  ...
                  {selectedProjectRef && <div className="install-form-hint" style={{ marginTop: 8 }}>Projeto selecionado: <strong>{selectedSupabaseUrl}</strong></div>}
                </div>
              </>
            )}
            ...
            {vercelProjects.length > 0 && (
              <div className="install-form-group">
                ...
                <select className="install-form-select" value={vercelProjectId} onChange={e => setVercelProjectId(e.target.value)}>
                  <option value="">Nao configurar (opcional)</option>
            ...
            <div className="install-nav-buttons">
              <button className="install-btn-back" onClick={() => setStep(1)}><ArrowLeft size={16} /> Voltar</button>
